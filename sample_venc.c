...
HI_S32 SAMPLE_VENC_1080P_CLASSIC(HI_VOID)
{
    ...
    PIC_SIZE_E enSize[3] = {PIC_HD1080, PIC_VGA,PIC_QVGA}; //图片像素
	HI_U32 u32Profile = 0;
    ...
    ******************************************/
	...
	s32ChnNum = 1;//只是单纯的做网络传输，所以只要一路就ok
	printf("s32ChnNum = %d\n",s32ChnNum);
    ...
    ******************************************/
    stViConfig.enViMode   = SENSOR_TYPE;//sensor类型
    stViConfig.enRotate   = ROTATE_NONE;//翻转
    stViConfig.enNorm     = VIDEO_ENCODING_MODE_AUTO;
    stViConfig.enViChnSet = VI_CHN_SET_NORMAL;//图像处理，镜像与翻转
    stViConfig.enWDRMode  = WDR_MODE_NONE;//宽动态，处理亮暗不一
    ...
    ******************************************/
	...
	if(s32ChnNum >= 1)
	{
	    ...
	    stVpssGrpAttr.enPixFmt = PIXEL_FORMAT_YUV_SEMIPLANAR_420;//输出图像模式
...
HI_S32 SAMPLE_VENC_1080P_MJPEG_JPEG(HI_VOID)
{
    ...
    ******************************************/
    ...
    stViConfig.enRotate   = ROTATE_NONE;  //旋转
    stViConfig.enNorm     = VIDEO_ENCODING_MODE_AUTO;
    stViConfig.enViChnSet = VI_CHN_SET_NORMAL;//图像处理，四种模式，镜像与翻转
	//stViConfig.enWDRMode  = WDR_MODE_NONE;//宽动态，曝光度亮暗程度不一
